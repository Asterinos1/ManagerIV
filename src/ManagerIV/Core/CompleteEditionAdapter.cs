...
public class CompleteEditionAdapter : IBackendAdapter
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="CompleteEditionAdapter"/> class.
    /// </summary>
    /// <param name="gameDir">The target GTA IV game directory path.</param>
    /// <param name="linker">The file system linker implementation to use.</param>
    /// <param name="journal">An optional transaction journal to record mutated operations.</param>
    ...
    /// <summary>
    /// Detects the game version and profile from the main game executable.
    /// </summary>
    /// <param name="gameDir">The game directory path to inspect.</param>
    /// <returns>A <see cref="GameVersionProfile"/> representing the game version.</returns>
    ...
    /// <summary>
    /// Resolves the deployment target for a given mod file.
    /// </summary>
    /// <param name="file">The mod file to inspect.</param>
    /// <returns>The resolved <see cref="DeployTarget"/>.</returns>
    ...
    public async Task<LoadOrderModel> ReadLoadOrderAsync()
    {
        ...
        if (_linker.DirectoryExists(pluginsDir))
        {
            var files = await Task.Run(() => Directory.GetFiles(pluginsDir));
            foreach (var file in files)
            {
                string fileName = Path.GetFileName(file);
                // Look for files with priority prefix like "05_ModName.asi"
                if (fileName.Length > 3 &&
                    char.IsDigit(fileName[0]) &&
                    char.IsDigit(fileName[1]) &&
                    fileName[2] == '_')
                {
                    string priorityStr = fileName.Substring(0, 2);
                    if (int.TryParse(priorityStr, out int priority))
                    {
                        string originalName = fileName.Substring(3);
                        entries.Add(new LoadOrderEntry(originalName, DeployTarget.Plugins, priority));
                    }
                }
            }
        }
    ...
    /// <summary>
    /// Gets the list of file paths to log files generated by backend tools.
    /// </summary>
    /// <returns>A collection of absolute log file paths.</returns>
